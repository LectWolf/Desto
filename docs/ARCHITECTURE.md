...
Application Card 的目录所有权、Shell 解析和失败语义记录在 [APPLICATION_CARD.md](APPLICATION_CARD.md)。Explorer 文件拖入由宿主收集路径，Storage Adapter 先完成可回滚移动，再重新生成项目视图；窗口消息过程不直接修改领域状态或执行零散文件操作。
